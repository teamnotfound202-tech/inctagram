...
export const VerificationLinkExpired = () => {
    ...
    const {register, handleSubmit, formState: {errors, isValid}, reset} = useForm<VerificationFormValues>();
    const [resendingEmail] = useRegistrationEmailResendingMutation()
    const [isModalOpen, setIsModalOpen] = useState(false)
    const [emailValue, setEmailValue] = useState('')
    const router = useRouter();

    const onSubmit: SubmitHandler<VerificationFormValues> = (data: {email: string}) => {
        const newData = {
            email: data.email,
            baseUrl: baseUrl + '/verify-email'
        }
        resendingEmail(newData)
            .then(() => {
                setIsModalOpen(true)
